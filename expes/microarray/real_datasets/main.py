...
from deconv.utils import mae, rmse, quantile_normalize, clean_mixture_signature
...
list_datasets = ["abbas", "becht", "gong", "kuhn", "newman_fl", "newman_pbmc", "shen", "shi"]
# list_datasets = ["abbas"]
...
def parallel_function(dataset, competitor):
    signature = pd.read_csv(path_to_dir + dataset + "/Signature.txt",
        header=0, index_col=0, delimiter="\t")
    mixture = pd.read_csv(path_to_dir + dataset + "/Mixture.txt",
        header=0, index_col=0, delimiter="\t")
    reference = pd.read_csv(path_to_dir + dataset + "/Reference.txt",
        header=0, index_col=0, delimiter="\t")
    phenotype = pd.read_csv(path_to_dir + dataset + "/Phenotype.txt",
        header=0, index_col=0, delimiter="\t")
    ...
    if competitor == "ssvr":
        ...
        pandas2ri.activate()
        results = robjects.r('EPIC_perso')(reference, signature, mixture, phenotype)
        estimated_prop = np.array(results)
        pandas2ri.deactivate()
        ...
        pandas2ri.activate()
        results = robjects.r('hspe_perso')(reference, signature, mixture, phenotype)
        estimated_prop = np.array(results)
        pandas2ri.deactivate()
    
    elif competitor == "dtangle":
        robjects.r['setwd'](path_to_dir)
        robjects.r.source("/Users/quentin.klopfenstein/Documents/deconvolution_SSVR/Analysis/deconv/expes/rnaseq/dtangle.R")
        pandas2ri.activate()
        results = robjects.r('dtangle_perso')(reference, signature, mixture, phenotype)
        estimated_prop = np.array(results)
        pandas2ri.deactivate()
...
results = Parallel(n_jobs=n_jobs, verbose=100, backend='loky')(delayed(parallel_function)(
        dataset, competitor)
