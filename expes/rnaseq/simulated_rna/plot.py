...
list_methods = ["SSVR", "Cibersort", "SOLS", "DeconRNAseq", "FARDEEP"]
...
fig, axarr = plt.subplots(
    2, 2, sharex=False, sharey=False, figsize=[20, 16])
for idx, noise in enumerate(list_noise):
    ...
    axarr[idx, 0].set_xticklabels(list_methods)
    ...
    im, cbar = heatmap(results_cells.T, list_methods, np.array(list_cells), ax=axarr[idx, 1],
...
for idx, noise in enumerate(list_noise):
    ...
    axarr2[idx, 0].set_xticklabels(list_methods)
    ...
    im, cbar = heatmap(results_cells.T, list_methods, np.array(list_cells), ax=axarr2[idx, 1],
...
fig2.tight_layout()
if save_fig:
    fig2.savefig("simulated_rna_mae.pdf")
fig2.show()