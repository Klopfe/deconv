...
list_competitors = ["ssvr", "cibersort", "sols",  "fardeep", "hspe", "EPIC", "deconrnaseq"]
# list_competitors = ["ssvr", "cibersort"]
...
path_to_dir = "/Users/quentin.klopfenstein/Documents/deconvolution_SSVR/Data/rnaseq/"

dict_ground_truth = {}
dict_ground_truth["linsley"] = np.array(pandas.read_csv(path_to_dir + "linsley" + "/Ground_truth.txt",
        header=0, index_col=0, delimiter="\t"))

dict_ground_truth["liu"] = np.array(pandas.read_csv(path_to_dir + "liu" + "/Ground_truth.txt",
        header=0, index_col=0, delimiter="\t"))
dict_ground_truth["parsons"] = np.array(pandas.read_csv(path_to_dir + "parsons" + "/Ground_truth.txt",
        header=0, index_col=0, delimiter="\t"))
dict_ground_truth["PBMC"] = np.array(pandas.read_csv(path_to_dir + "PBMC" + "/Ground_truth.txt",
        header=0, index_col=0, delimiter="\t"))
dict_ground_truth["racle"] = np.array(pandas.read_csv(path_to_dir + "racle" + "/Ground_truth.txt",
        header=0, index_col=0, delimiter="\t"))
...
for idx, competitor in enumerate(list_competitors):
    ...
    for idx2, dataset in enumerate(list_datasets):
        ...
        # if dataset == "linsley" and (competitor is not "hspe") and (competitor is not "EPIC"):
        #     estimated_prop[:, 0] = np.sum(estimated_prop[:, 0:4], axis=1)
        #     estimated_prop = np.delete(estimated_prop, (1, 2, 3), axis=1)
...
list_methods = ["AC-SVR", 'Cibersort', 'SOLS', 'FARDEEP', "HSPE", "EPIC", "DeconRNA"]
...
fig.tight_layout()
if save_fig:
    fig.savefig("/Users/quentin.klopfenstein/Documents/deconvolution_SSVR/Manuscript/figures/real_rnaseq.pdf")
fig.show()
...
# figlegend.savefig("/Users/quentin.klopfenstein/Documents/deconvolution_SSVR/Manuscript/figures/legend_real_rnaseq.pdf")
